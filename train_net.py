...
from d2fp import (
    ...
    ParsingWithTTA,
    D2FPSemanticHPDatasetMapper,
    D2FPParsingDatasetMapper,
    D2FPParsingLSJDatasetMapper,
    ParsingEvaluator,
    WandBWriter,
    add_d2fp_config,
...
from d2fp.modeling.backbone.utils import get_vit_lr_decay_rate
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "parsing":
            evaluator_list.append(
                ParsingEvaluator(
                    dataset_name,
                    cfg.MODEL.D2FP.TEST.PARSING_INS_SCORE_THR,
                    output_dir=output_folder,
                    parsing_metrics=cfg.MODEL.D2FP.TEST.METRICS
                )
            )
    ...
    def build_train_loader(cls, cfg):
        ...
        if cfg.INPUT.DATASET_MAPPER_NAME == "d2fp_semantic_hp":
            mapper = D2FPSemanticHPDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        # multiple human parsing dataset mapper
        elif cfg.INPUT.DATASET_MAPPER_NAME == "d2fp_parsing":
            mapper = D2FPParsingDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        # multiple human parsing dataset mapper with lsj
        elif cfg.INPUT.DATASET_MAPPER_NAME == "d2fp_parsing_lsj":
            mapper = D2FPParsingLSJDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        else:
            mapper = None
            return build_detection_train_loader(cfg, mapper=mapper)
    ...
    def test_with_TTA(cls, cfg, model):
        ...
        if cfg.MODEL.D2FP.TEST.PARSING_ON:
...
def setup(args):
    ...
    # Setup logger for "d2fp" module
    setup_logger(output=cfg.OUTPUT_DIR, distributed_rank=comm.get_rank(), name="d2fp")
    return cfg
